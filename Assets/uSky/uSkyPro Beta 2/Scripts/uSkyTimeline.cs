	...
	public class uSkyTimeline : MonoBehaviour {
		...
		void SetTimelineState ()
		{
			...
			case TimeSettingsMode.Realistic:
				uSkyInternal.SetTimelineSettingState (locationAndDate);
				break;
			default:
				uSkyInternal.SetTimelineSettingState (sunAndMoon);
				break;
			}
		}

		// Startup Initialization, no event triggering
		void InitSunAndMoonDirection ()
		{
			switch (Type) {
			case TimeSettingsMode.Realistic:
		...
		void UpdateSunAndMoonDirection ()
		{
			switch (Type) 
			{
			case TimeSettingsMode.Realistic:
...
// DEFAULT SETTINGS : SUN AND MOON 
		...
		void SetSunAndMoonDirection() 
		{
			...
			if (m_Sun) 
			{
				...
				uSkyInternal.SetSpaceAndStarsRotation (sunEuler);	// space and stars
			}

			if ((uSkyInternal.m_NightSkyMode == 1 || uSkyPro.instance == null) && m_Moon)
			{
...
//			Debug.Log ("Sun and Moon direction have been updated");
		}
		#endregion
...
// Properties accessor --------------------
		...
		public float Longitude 
		{
			...
			set { 
					...
					if (value < -180.0f)
						value = 180.0f;
					locationAndDate.longitude = value;
				}
		}
